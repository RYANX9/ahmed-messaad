...
    const INITIAL_SIZE = 240; // Matches the fixed width/height in the animated img style
...
      // Calculate absolute top-left coordinates of the target (accounting for scroll)
      const absoluteLeft = rect.left + window.scrollX;
      const absoluteTop = rect.top + window.scrollY;

      // --- OPTIMIZED ANIMATION LOGIC (Transform/Scale) ---

      // 1. Calculate the center-to-center translation needed in pixels
      // Initial center is (window.innerWidth / 2, window.innerHeight / 2)
      const targetCenterX = absoluteLeft + rect.width / 2;
      const targetCenterY = absoluteTop + rect.height / 2;

      const translateX_move = targetCenterX - (window.innerWidth / 2);
      const translateY_move = targetCenterY - (window.innerHeight / 2);

      // 2. Calculate the scale needed from the initial size (240px)
      const scaleX = rect.width / INITIAL_SIZE;
      const scaleY = rect.height / INITIAL_SIZE;

      // 3. Set the final transform using translate (for movement) and scale (for size change).
      // We combine the initial centering, the move, and the scale into one GPU-accelerated property.
      const finalTransform = `translate(-50%, -50%) translate(${translateX_move}px, ${translateY_move}px) scaleX(${scaleX}) scaleY(${scaleY})`;

      animatedImg.style.transform = finalTransform;
      
...
          borderRadius: '16px',
          // Optimized: translate(-50%, -50%) centers it, translateZ(0) forces GPU layer
          transform: 'translate(-50%, -50%) translateZ(0)', 
          zIndex: 100,
          pointerEvents: 'none',
          // We only transition the 'transform' property for maximum smoothness
