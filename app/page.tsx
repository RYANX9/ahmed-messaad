...
export default function Page() {
  ...
  // NEW: State to manage the GPU-accelerated transform for the animation.
  // Initial value centers the image (translate(-50%, -50%)).
  const [animatedTransform, setAnimatedTransform] = useState('translate(-50%, -50%)');
  ...
  React.useEffect(() => {
    ...
    const initialDelay = setTimeout(() => {
      ...
      // --- FIX START: Calculate transform for smooth animation ---
      const rect = profileSection.getBoundingClientRect();
      const initialSize = 240; // Initial hardcoded width/height from inline style

      // 1. Calculate the center of the target grid box (viewport coordinates)
      const targetCenterX = rect.left + rect.width / 2;
      const targetCenterY = rect.top + rect.height / 2;

      // 2. Initial center of the screen (viewport coordinates)
      const initialCenterX = window.innerWidth / 2;
      const initialCenterY = window.innerHeight / 2;
      
      // 3. Calculate the translation needed to move the center point
      const translateX = targetCenterX - initialCenterX;
      const translateY = targetCenterY - initialCenterY;

      // 4. Calculate the scale factor
      const scaleX = rect.width / initialSize;
      const scaleY = rect.height / initialSize;

      // 5. Construct the final transform string
      // Order: Translate to final center, then Scale.
      const finalCenterTransform = `translate(${translateX}px, ${translateY}px) scale(${scaleX}, ${scaleY})`;
      
      // Use requestAnimationFrame to ensure the transform is applied on the next frame, 
      // preventing the initial jump/flicker.
      requestAnimationFrame(() => {
        setAnimatedTransform(finalCenterTransform); // This triggers the smooth CSS transition

        // After animation completes, hide animated and show static (duration 1500ms)
        const completeDelay = setTimeout(() => {
          // Use DOM to hide the element since it's not managed by a state
          const imgToHide = document.getElementById('animated-profile');
          if (imgToHide) {
    ...
    <main className="bg-[#0a0a0a] text-white min-h-screen overflow-x-hidden">
      ...
      <img
        ...
        style={{
          ...
          zIndex: 100,
          pointerEvents: 'none',
          // FIX: Use transform state to transition a single GPU-accelerated property
          transition: 'all 1500ms cubic-bezier(0.25, 0.46, 0.45, 0.94)', 
          transform: animatedTransform, // Use the state here
        }}
      />
